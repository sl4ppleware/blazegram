...
async fn main() {
    ...
    App::builder(&token)
        ...
        .callback("list", |ctx| Box::pin(async move {
            ctx.navigate(list_screen(&ctx.state::<TodoState>())).await
        }))
        .callback("add", |ctx| Box::pin(async move {
            ctx.navigate(
        ...
        .callback("clear", |ctx| Box::pin(async move {
...
fn list_screen(st: &TodoState) -> Screen {
    ...
    let lines: String = st.items.iter().enumerate()
        .map(|(i, item)| format!("{}. {}", i + 1, blazegram::markup::escape(item)))
        .collect::<Vec<_>>()
        .join("\n");

    Screen::text("list", format!("<b>To-Do</b> ({})\n\n{lines}", st.items.len()))
        .keyboard(|kb| {
            let mut kb = kb;
            for (i, _) in st.items.iter().enumerate() {
                kb = kb.button(&format!("\u{2716} {}", i + 1), &format!("del:{i}"));
                if (i + 1) % 4 == 0 { kb = kb.row(); }
            }
            kb.row()
              .button("+ Add", "add").button("Clear all", "clear").row()
        })
        .build()
}
